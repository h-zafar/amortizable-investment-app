...
const FunnelEdit = () => {
  ...
  useEffect(() => {
    if (id) {
      axios
        .get(`http://localhost:1337/api/funnels/${id}`)
        .then((res) => {
          setData(res.data);
          setIsLoading(false);
        })
  ...
  const loadData = () => {
    let d = data.data;
    const { Name, Priority, conditions } = d.attributes;
    const {
      type,
      rules,
      rulesSeq,
    ...
    setTitle(Name || DEFAULT_FUNNEL_TITLE);
    setSchemas(rules || DEFAULT_FUNNEL_RULES);
    setRuleType(type || DEFAULT_FUNNEL_RULE_TYPE);
    setOffer(offer || DEFAULT_OFFER);
    setOfferAccept(offerAccept || DEFAULT_OFFER_ACCEPT);
    setOfferDecline(offerDecline || DEFAULT_OFFER_DECLINE);
    setOfferVariants(offerVariants || DEFAULT_OFFER_VARIANTS);
    setOfferLimitToVariants(
      offerLimitToVariants || DEFAULT_OFFER_LIMIT_TO_VARIANTS
    );
    setOfferHideVariantsSelector(
      offerHideVariantsSelector || DEFAULT_OFFER_HIDE_VARIANT_SELECTOR
    );
    setOfferPricing(offerPricing || DEFAULT_OFFER_PRICING);
    setOfferSettings(offerSettings || DEFAULT_OFFER_SETTINGS);
    setTimerSettings(timerSettings || DEFAULT_TIMER_SETTINGS);
    setOfferTitleSettings(offerTitleSettings || DEFAULT_OFFER_TITLE_SETTINGS);
    setProductDetailsSettings(
      productDetailsSettings || DEFAULT_PRODUCT_DETAILS_SETTINGS
    );
    setLayoutEditorCards(layoutEditorCards || DEFAULT_LAYOUT_EDITOR_CARDS);
  };
  ...
  const handleRuleUpdate = (i, data) => {
    console.log("UPDATED DATA in Cart Content: ", data);

    let currSchemas = schemas;
    currSchemas[i] = data;
    setSchemas(currSchemas);

    console.log("SCHEMAs after update: ", schemas);
  };

  const handleSave = async () => {
    console.log("SCHEMAS inside handleSave : ", schemas);
    ...
    console.log("DATA for PUT (Edit): ", data);

    const res = await axios.put(`http://localhost:1337/api/funnels/${id}`, {
      data: { Name: title, Priority: 1, conditions: data },
    });

    console.log("PUT (Edit) res: ", res);
    ...
    return (
      <div className="m-4">
        <LegacyCard sectioned>
          <div className="p-4">
            <Text color="critical">Error: Something went wrong!</Text>
          </div>
          <Button primary onClick={() => navigate("/funnels")}>
            Go Back
          </Button>
        </LegacyCard>
      </div>
  ...
  return (
    <>
      {mode === "FUNNEL" && (
        ...
        >
          <TitleBar title="Edit Funnel" breadcrumbs={breadcrumbs}></TitleBar>
